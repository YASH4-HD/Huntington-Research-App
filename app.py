...
with tab3:
    ...
    if st.button("Generate Scientific Summary"):
        summary = f"Analysis of the {disease_choice} framework identifies {res_df.iloc[0]['Mechanism']} as a statistically significant driver (p = {res_df.iloc[0]['Adj. P-Value']:.4e})."
        st.markdown(summary)
