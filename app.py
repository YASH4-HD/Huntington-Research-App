...
if not df.empty:
    ...
    # Calculate global score for all tabs
...
with tab2:
    ...
    with col_stats:
        ...
        if G.number_of_nodes() > 0:
            ...
            st.caption("🔍 Higher connectivity indicates coordinated dysfunction across metabolic modules.")
            st.write("---")
        ...
        st.info("💡 PSMC subunits indicate proteasome overload and impaired protein clearance.")
...
with tab3:
    ...
    # Calculation based on the FULL dataset for consistency
    N = 20000
    n_sample = 50 # Top 50 genes
    ...
    with c_right:
        # Restore the tall bars by plotting -log10(p)
        st.bar_chart(data=res_df, x="Mechanism", y="-log10(p)")
        st.caption("Higher bars = Higher statistical significance (-log10 p-value)")
