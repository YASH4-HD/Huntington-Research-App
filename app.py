...
with tab2:
    ...
    with col_stats:
        ...
        # k=2.5 significantly increases the distance between nodes
        # iterations=200 gives the simulation more time to resolve overlaps
        pos = nx.spring_layout(G, k=2.5, iterations=200, seed=42)
        
        for role, color in role_colors.items():
            nodes = [n for n, attr in G.nodes(data=True) if attr.get('role') == role]
            if nodes:
                # Reduced node size slightly (150 -> 120) to create more white space
        ...
        # font_size=5 is the "sweet spot" for dense clusters
        # verticalalignment='bottom' moves the text slightly off the center of the node
